...
export function assignments(state = assignmentsDefaultState, action) {
  switch (action.type) {
    ...
    case actionTypes.ASSIGNMENT_CLOSED:
      let close_index = state.openAssignments.findIndex(x => x.woID === action.woID);
      ...
      let tabIdx = state.openAssignmentsTabIdx.findIndex(x => x === action.woID);
      ...
      if (caseIdx === -1 && !action.woID.includes(" "))
