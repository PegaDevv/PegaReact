...
import { ReferenceHelper, htmlDecode, getImageInfo, isTrue, dateToPegaDateValue, getUniqueIndex } from "../_helpers";
...
class PegaForm extends Component {
    ...
    return (
      ...
      >
        ...
        <Segment attached="bottom" style={{ overflow: "hidden" }}>
          <Button.Group floated="left" style={{backgroundColor:"green"}}>
          ...
          <Button.Group floated="right" style={{backgroundColor:"red"}}>
            ...
            {bScreenFlow && (
              <Button style={{backgroundColor:"green"}} type="submit" primary>{bLastStep ? fnGetLabel(sLastStepActionID) : fnGetLabel(formButtonActionIDs.NEXT)}</Button> 
            )}
            {!bScreenFlow && (
              <Button style={{backgroundColor:"green"}} type="submit" primary>{fnGetLabel(formButtonActionIDs.SUBMIT)}</Button> 
            )}
          </Button.Group>
        </Segment>
      </Form>
  ...
  saveRepeatLayoutInfo(layout) {
    ...
    // RDLs don't have a reference value presently.  They both have a fieldListID value.
  ...
  // TODO: Keep a stack of groupFormats being created...and as creating a field store with the field the
  //  containing significant repeat groupFormat (by walking back the stack and finding what type of repeat
  ...
  createGrid(layout, index = 0) {
    ...
    const bEditRows = layout.repeatRowOperations && layout.repeatRowOperations.rowEditing !== rowEditingTypes.READONLY && layout.readOnly !== true;
    ...
    let gridMarkup = (
      <Table compact key={index}>
        ...
        <Table.Body>
          {rows.map((row) => {
            return (
              <Table.Row key={getUniqueIndex()} className="pr-grid-row">
                {row.groupIndex && (
  ...
  createField(field, index, showLabel) {
    ...
    // If in a repeat construct of type "Dynamic" (RDL), then cope with missing reference (hopefully fixed
    //  with future Pega releat patches)
    let repeatLayoutType = null;
    if( this.repeatLayoutTypeStack.length > 0 && 
        (repeatLayoutType = this.repeatLayoutTypeStack[this.repeatLayoutTypeStack.length-1]) === layoutTypes.DYNAMIC ) {
    ...
    let labelFormat = this.getLabelFormat(field);
    switch (field.control.type) {
      case fieldTypes.CHECKBOX:
        debugger
        ...
        if (readOnly) {
          ...
          fieldElem = (
              <Form.Field key={index} required={required} disabled={disabled} error={error}
          ...
          fieldElem = (
            ...
            <Form.Checkbox key={index} required={required} disabled={disabled} error={error} className={fieldClass}
              name={field.reference}
      ...
      case fieldTypes.LOCALRADIO:
        ...
        if (readOnly) {
          ...
          if( checked ) {
            let oRefInfo = ReferenceHelper.getTargetAndIndex(field.reference);
            if( oRefInfo.target ) {
              this.oLocalRadio[oRefInfo.target] = field.reference;
            }
          }
          fieldElem = (
      ...
      case fieldTypes.TEXTINPUT:
        if (readOnly) {
          ...
          fieldElem = (
            <Form.Input
              //className={fieldClass}
              className={`${labelFormat} ${fieldClass}`}
              key={index}
              required={required}
              disabled={disabled}
              name={field.reference}
      ...
      case fieldTypes.LABEL:
        fieldElem = (
          <Label key={index} size="large" data-test-id={field.testID}>
            {label}
          </Label>
        );
        break;
 ...
 */
    ...
    return (
      <div key={index} className={`readonlytext ${fieldClass}`} data-test-id={field?.testID}>
        ...
        {displayValue && displayValueClasses.length > 0 && (
           <span className={displayValueClasses.join(' ')}>{displayValue}</span>
        )}
        {displayValue && displayValueClasses.length===0 && (
            <>{displayValue}</>
        )}
  ...
  getLabelFormat(field) {
    let labelFormat = null;
    if(field.labelFormat) {
      labelFormat = "pr-label-format-" + field.labelFormat.replaceAll(" ","-");
      return labelFormat;
    }
  }
  ...
  */
  ...
  handleChange(e, obj, callback = null, field) {
    ...
    if( field && field.control?.type == fieldTypes.DATETIME) {
      ...
      // Handle inputs for checkboxes and local radio buttons (but exclude regular radio buttons)
      if( field && ( field.control?.type === fieldTypes.CHECKBOX || field.control?.type === fieldTypes.LOCALRADIO )) {
        if( field.control.type === fieldTypes.LOCALRADIO && obj.checked ) {
          // If setting a new radio button, make sure the prior selected value is now false
          let oRefInfo = ReferenceHelper.getTargetAndIndex(field.reference);
          let sCheckedRef = this.oLocalRadio[oRefInfo.target];
    ...
    if( sPriorSetLocalRadioRef ) {
      updatedState.values[sPriorSetLocalRadioRef] = "false";
    }
    this.setState(
      updatedState,
      callbackFunc
  ...
  addRowAction(layout, rowRef, groupRowRef=null, bAppend=false) {
    ...
    if (sRefType == "List") {
      ...
      while (!bGotGoodIndex) {
        groupRowRef = prompt("Enter a Row name that doesn't exist to add (alpha first char)", "");
        if( null === groupRowRef ) {
          // Cancel
          break;
        } else if( groupRowRef !== "" && isNaN(parseInt(groupRowRef, 10)) ) {
            // checking whether there exists a row with same name as groupRowRef(entered value in prompt)
  ...
  addRemoveRow(sAction, layout) {
    ...
    if (sRowEditing !== rowEditingTypes.READONLY) {
      ...
      if (layout.referenceType === "List") {
        ...
        while( !bGotGoodIndex ) {
          rowName = prompt(bAdd ? "Enter a Row name that doesn't exist to add (alpha first char)" : "Enter a Row name that exists to remove","");
          if( null === rowName ) {
            break;
          } 
          else if( rowName !== "" && isNaN(parseInt(rowName,10)) ) {
            // checking whether there exists a row with same name as rowName(entered value in prompt).
            if(bAdd){
...
debugger
  ...
  handleSave(e, data) {
    ...
    let newValues = Object.assign({}, this.state.values);
    let pi = this.bUsingAnyPI ? Object.assign({}, this.pi.getPageInstructions()) : null;
  ...
  handleCaseCreate(model, reset, invalidateForm) {
    ...
    let postContent = ReferenceHelper.getPostContent(this.state.values, this.pi.getPostSettings());
    this.props.dispatch(caseActions.createCase(this.state.processID, postContent))
    .then( action =>{
    })
    .catch( error => {
  ...
  render() {
    ...
    // Also show caseView on the right side of the WorkObject (if so configured)
    let bShowDetails = this.props.appSettings.bShowCaseDetails;
    let index=0;
    return (
      <div>
      ...
      <Grid columns={2} stackable as={Segment} attached="bottom" key={index}>
        <Grid.Row>
          <Grid.Column width={bShowDetails?10:16}>
