...
import { getError, getCreateCaseContext } from "../_helpers";
...
function getCaseCreationPage(id, processID) {
  ...
  function success(woID, id, data, processID) {
    return { type: actionTypes.CASE_CREATION_PAGE_SUCCESS, woID, id, data, processID };
  }
  function failure(error) {
  ...
  return dispatch => {
    ...
    return caseService.createCase(id, processID, content).then(
      aCase => {
        ...
        const woID = aCase.ID;
        dispatch(assignmentActions.addOpenAssignment(woID, aCase.ID, aCase.nextAssignmentID));
        dispatch(getCase(woID, aCase.ID));
        dispatch(assignmentActions.getAssignment(woID, aCase.nextAssignmentID));
        // Below causes newly created tab to never display
        //dispatch(assignmentActions.closeAssignment(woID, id));
