...
import { getError, isShowCaseDetails } from "../_helpers";
  ...
  return dispatch => {
    ...
    return assignmentService.getAssignment("next").then(
      assignment => {
        ...
        dispatch(getFieldsForAssignment(woID, assignment, assignment.caseID)).then(data => {
...
function getFieldsForAssignment(woID, assignment, actionId = null, bIsLocalAction=false) {
  return dispatch => {
    ...
    if( isShowCaseDetails() ) {
      dispatch(caseActions.getView(woID, assignment.caseID, "pyCaseDetails"));
    }
      ...
      .then(
        assignment => {
          if (assignment.nextAssignmentID) {
            ...
            dispatch(caseActions.getView(woID, caseID, "pyCaseDetails"));
...
function closeAssignment(woID) {
  return { type: actionTypes.ASSIGNMENT_CLOSED, woID };
}
