...
export const initEndpoints = () => {
    if(endpoints.PEGAURL && !endpoints.bInitialized) {
        if( !endpoints.BASEURL ) {
            endpoints.BASEURL = endpoints.PEGAURL + "/api/v1";
        }
        if( endpoints.use_v2apis && !endpoints.BASEV2URL ) {
            endpoints.BASEV2URL = endpoints.PEGAURL + "/api/application/v2";
        }
...
const initPegaOAuth = () => {
    ...
    if( !sSI ) {
        ...
        const sAuthPagePath = window.location.origin + homeUrl + 
            (-1 === window.location.origin.indexOf("localhost") ? "" : "auth");
