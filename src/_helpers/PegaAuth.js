class PegaAuth {
    ...
    async buildAuthorizeUrl(state) {
        ...
        const moreAuthArgs =
            ...
            (userIdentifier ? (`&UserIdentifier=${encodeURIComponent(userIdentifier)}` +
                (password ? `&Password=${encodeURIComponent(window.atob(password))}` : '')) : '' );
    ...
    async login() {
        let fnGetRedirectUriOrigin = () => {
            let redirectUri = this.config.redirectUri;
            let nRootOffset = redirectUri.indexOf("//");
            let nFirstPathOffset = nRootOffset !== -1 ? redirectUri.indexOf("/",nRootOffset+2) : -1;
            return nFirstPathOffset !== -1 ? redirectUri.substring(0,nFirstPathOffset) : redirectUri;
        };

        let redirectOrigin = fnGetRedirectUriOrigin();
        let state = window.btoa(location.origin);
        ...
        return new Promise( (resolve, reject) => {
            ...
            this.buildAuthorizeUrl(state).then((url) => {
                ...
                let tmrAuthComplete = null, checkWindowClosed = null;
                let myWinOnLoad = () => {
                    try{
                        if( bWinIframe ) {
