...
class Workarea extends Component {
  componentDidMount() {
    this.props.dispatch(userActions.getUserData(this.props.appSettings.bshowWorkgroupBaskets));
  }

  componentDidUpdate(prevProps) {
    if (prevProps.appSettings.bshowWorkgroupBaskets != this.props.appSettings.bshowWorkgroupBaskets) {
      this.props.dispatch(userActions.getUserData(this.props.appSettings.bshowWorkgroupBaskets));
    }
  }
  ...
  getPanes() {
    ...
    this.props.openAssignments.forEach(x => {
      // Use the top case id as the caseID attribute value for WorkObject and PegaForm
      let caseID = x.woID;
      let assignmentID = x.ID ? x.ID : x.caseID;
      if (
